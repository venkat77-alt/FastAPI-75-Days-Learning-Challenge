...
from schemas.response_schema import productsResponse,productcreateresponse
from fastapi import HTTPException
from schemas.request_schema import createproduct,updateproduct
...
async def get_products(
        ...
        paginations=Depends(pagination)
):

    product=await get_products_service(search,category,min_price,max_price,paginations=paginations)

    return product


@router.get("/{id}",response_model=productsResponse)
async def get_product(id:int=Path(..., description="The ID of the product to retrieve",
                    ge=1, le=9999),
